...
class DaDaCoinBlockChain:
    ...
    def new_block(self,
                  previous_hash: Optional[str]) -> Dict[str, Any]:  # 创建一个区块，返回字典数据类型  传入一个previous_hash参数作为前一个区块的哈希值，返回一个键值对Dict[str, Any]类型的新区块。
        block = {
        ...
        self.current_transactions = []  # 将当前交易列表清空，以备下一次新区块的交易记录存储。
    ...
    def new_transactions(self, sender: str, recipient: str, amount: int) -> int:  # 创建一个交易
        ...
        return self.last_block["index"] + 1  # 返回最后一个区块的索引值加1。这个值用于标识新交易在区块链中的位置。   索引标记交易的数量

    @property # 装饰器 将方法last_block转化为属性
    def last_block(self) -> Dict[str, any]:  # 取得最后一个区块
        return self.chain[-1] #返回区块链中的最后一个区块。self.chain是区块链对象的属性，通过索引[-1]获取最后一个区块。

    @staticmethod #hash静态方法 可以在不实例化对象的情况下直接调用
    ...
    def valid_chain(self, chain: List[Dict[str, any]]) -> bool:  # 区块链校验
        ...
        last_block = chain[0]  # 第一个区块 将作为在区块链上遍历的当前块的索引
    ...
    def resolve_conflicts(self) -> bool:  # 共识算法 接受一个链，并返回一个布尔值  解决不同节点之间链的冲突。如果发现新区块链，它将替换当前节点上的区块链
        # 网络中的多个节点，取出最长的
        neighbours = self.nodes  # 取得所有节点
        new_chain = None  # 新的区块链
        max_length = len(self.chain)  # 将max_length初始化为当前的区块链长度
        for node in neighbours: #循环处理每个节点
...
def new_node():
    values = request.get_json(force=True)  # 获取json字符串
    nodes = values.get("nodes")  # 获取所有的节点
    if nodes is None:
        return "没有节点信息", 400
    for node in nodes: # 遍历节点列表，调用dadacoin对象的register_node方法，向区块链网络中增加新的节点。
